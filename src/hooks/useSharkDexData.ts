...
export function formatCurrency(value?: number | string | null) {
  ...
  if (!Number.isFinite(numericValue)) return 'Waiting';
...
export function shortenAddress(address: string) {
  if (!address) return 'Official CA will be added at launch';
...
export function useSharkDexData() {
  ...
  useEffect(() => {
    ...
    const fetchTokenPrice = async () => {
      try {
        ...
        console.error('Unable to load $SHARK DEX price:', err);
  ...
  const values = useMemo(() => {
    ...
    return {
      price: pair?.priceUsd ? formatCurrency(pair.priceUsd) : 'Waiting for CA',
      marketCap: marketCap ? formatCompactCurrency(marketCap) : 'Waiting for DEX',
      liquidity: pair?.liquidity?.usd ? formatCompactCurrency(pair.liquidity.usd) : 'Waiting for DEX',
      change24h:
        pair?.priceChange?.h24 !== undefined ? formatPercent(pair.priceChange.h24) : 'Waiting for DEX',
      volume24h: pair?.volume?.h24 ? formatCompactCurrency(pair.volume.h24) : 'Waiting for DEX',
      pairUrl: pair?.url || '',
    };
  }, [pair]);
